...
class Auth:  # pylint: disable=too-few-public-methods
    ...
    LOGIN_URL = (
        "https://www.volkswagen.de/app/authproxy/login?fag=vw-de,vwag-weconnect&scope-vw-"
    ...
    IDENTIFIER_URL = (
        "https://identity.vwgroup.io/signin-service/v1/4fb52a96-2ba3-4f99-a3fc-"
    ...
    def __do_login(self) -> bool:
        ...
        if (
            req.status_code != 200
            or not "SESSION" in req.cookies
            or not 'id="hmac"' in req.text
            or not 'id="csrf"' in req.text
            or not 'id="input_relayState"' in req.text
        ):
        ...
        if len(values) != len(search_values):
            ...
            return False
        hmac, csrf, relay_state = (
            values["hmac"],
            values["csrf"],
            values["input_relayState"],
        )
    ...
    def __get_values(fields: List[str], request_text: str) -> Dict:
        ...
        for field in fields:
            tmp = re.findall(
                'id="' + field + '"[^>]*value="([^"]*)"', request_text, re.IGNORECASE
            )
            if not tmp:
                tmp = re.findall(
                    'value="([^"]*)"[^>]*id="' + field + '"',
